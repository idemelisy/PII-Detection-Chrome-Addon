...
## New: Fill (faker) button

- A new "🪄 Fill (faker)" button was added to the extension UI. After you accept/redact detected PII (individual accepts or "✅ Accept All"), click "Fill (faker)" to replace redaction labels (for example, [NAME], [EMAIL], [PHONE]) or `.pii-redacted` spans with generated synthetic values.
- Generation is client-side (no backend changes required) and uses simple built-in generators for common PII types:
  - PERSON → fake full name
  - EMAIL → fake@example.com style address
  - PHONE → US-style phone number
  - LOCATION → street + city
  - ORGANIZATION → fake company
  - CREDIT_CARD, SSN, IP_ADDRESS, URL, DATE_TIME → synthetic values
- Notes:
  - For ChatGPT/Gemini, the fill operation updates the chat input safely (using the extension's React-compatible setter) so the generated values will be sent when you press Send.
  - For regular pages, `.pii-redacted` spans will be replaced with generated text.
  - If you want deterministic or localized fake data, I can add seeding or integrate a more full-featured faker library.
